...
export class ServiceClient {
	...
	async getStatus():Promise<github_com_janhalfar_vocablion_services_status.StatusState> {
		let response = await this.transport<{0:github_com_janhalfar_vocablion_services_status.StatusState; 1:github_com_janhalfar_vocablion_services.ServiceError}>("GetStatus", [])
		let err = response[1];
		if(err) { throw err }
		return response[0]
	}
